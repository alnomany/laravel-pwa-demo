...
return view('resources.resources',compact('titles','dates','links','country','name','type'));
    ...
    public function alainsport(){
        ...
        require '../vendor/autoload.php';
        $httpClient = new \simplehtmldom\HtmlWeb();
        $response = $httpClient->load('https://al-ain.com/section/sports/');
...
return view('resources.resources',compact('titles','dates','links','country','name','type'));
    ...
    public function skynewsarabia(){
        ...
        require '../vendor/autoload.php';
        $httpClient = new \simplehtmldom\HtmlWeb();
        $response = $httpClient->load('https://www.skynewsarabia.com/business/1458999-%D8%A7%D9%84%D8%B3%D8%B1-%D8%A7%D9%84%D8%B3%D9%87%D9%88%D9%84%D8%A9-%D8%AA%D8%AC%D8%B0%D8%A8-%D8%A7%D9%84%D8%A7%D9%95%D9%85%D8%A7%D8%B1%D8%A7%D8%AA-%D8%A7%D9%84%D8%B4%D8%B1%D9%83%D8%A7%D8%AA-%D9%88%D8%A7%D9%84%D9%85%D8%B3%D8%AA%D8%AB%D9%85%D8%B1%D9%8A%D9%86');

// echo the title
#echo $response->find('title', 0)->plaintext . PHP_EOL . PHP_EOL;
$titles=[];
foreach ($response->find('h2.sna_content_heading') as  $title) {
    $titles[] = $title->plaintext;
}
$dates = [];
foreach ($response->find('div.article_content_rev_cont div.sna_content_head_cont div.sna_content_head_date span.sna_content_head_date') as $date) {

    $dates[] = $date->plaintext;
}

$describations = [];
foreach ($response->find('div.article_content_rev_cont div.article_rev_sec_right_cont p.article-summary') as $describation) {

    $describations[] = $describation->plaintext;
}
return $titles;
...
$name=" skynewsarabia";
$type="أقتصادية";
return view('resources.resources',compact('titles','dates','country','name','type'));
...
return view('resources.resources',compact('titles','dates','links','country','name','type'));
    ...
    public function ir(){
        ...
        require '../vendor/autoload.php';
        $httpClient = new \simplehtmldom\HtmlWeb();
        $response = $httpClient->load('https://ir.voanews.com/');

// echo the title
#echo $response->find('title', 0)->plaintext . PHP_EOL . PHP_EOL;
$titles=[];
foreach ($response->find('h2.teaser__title span') as  $title) {
    $titles[] = $title->plaintext;
}
$links = [];
foreach ($response->find('div.teaser__content a') as $link) {
